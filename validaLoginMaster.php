<?php
// Conexión a mess_rrhh para LoginMaster (personal/jefes)
...
$id_usuario = $_POST['id_usuarioPRACT']    ?? $_POST['id_usuarioPRACT']    ?? '';
$nombre     = $_POST['nombredelusuarioPRACT'] ?? $_POST['nombredelusuarioPRACT'] ?? '';
$noEmpleado = $_POST['noEmpleadoPRACT']    ?? $_POST['noEmpleadoPRACT']    ?? '';
$rol        = $_POST['rolPRACT']           ?? $_POST['rolPRACT']           ?? '';
$usuario    = $_POST['correoPRACT']        ?? $_POST['correoPRACT']        ?? '';
...
if (empty($id_usuario) || empty($noEmpleado)) {
    ...
    if($nr == 1){
        ...
        echo '<script>document.cookie = "nombre='.$nombreEmpleado.';expires=" + new Date(Date.now() + 86400000).toUTCString() + ";SameSite=Lax;";</script>';
